...
class HeapPriorityQueue():
   ...
   def heapDown(self, k, size):
      ...
      if left==size and self.queue[k] > self.queue[left]:
         ...
         if self.queue[left]<self.queue[right]:
            ...
            maxC = right
         if self.queue[k]>self.queue[maxC]:
            self.swap(k, maxC)
   ...
   def remove(self, index):
      ...
      remvedVal = self.queue.pop()
      self.reheap()
      return remvedVal      # change this 
...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   # Your code goes here
   for each in open(nodes).readlines():
      index, lat, longer = each.rstrip().split(" ")
      nodeLoc[index] = (float(lat), float(longer))
   for each in open(node_city).readlines():
      string = ' '.join(each.rstrip().split(" ")[1:])
      nodeToCity[each.rstrip().split(" ")[0]] = string
      cityToNode[string] = each.rstrip().split(" ")[0]
   for edge in open(edges).readlines():
      efirst, esec = edge.rstrip().split(" ")
      if efirst in neighbors: 
         neighbors[efirst].add(esec)
      else:
         neighbors[efirst] = {esec}
      if esec in neighbors:
         neighbors[esec].add(efirst)
      else:
         neighbors[esec] = {efirst}
      calcost = calc_edge_cost(*nodeLoc[efirst], *nodeLoc[esec])
      edgeCost[(efirst, esec)] = calcost
      edgeCost[(esec, efirst)] = calcost
...
def display_path(path, graph):
   ...
   # Your code goes here
   print ([graph[1][i] for i in path if i in graph[1]])
   pass
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   ...
   for each in range(len(path)-1):
      drawLine(canvas, *graph[5][path[each]], *graph[5][path[each+1]], col)
      ROOT.update()
...
def bi_bfs(start, goal, graph, col):
   ...
   explored = {start: "s"}
   explored2 = {goal: "s"}
   while len(for_f)>0 and len(back_f)>0:
      temp = for_f[:]
      for_f = []
      while len(temp)>0:
         first = temp.pop(0)
         if first in back_f:
            p1, c1 = generate_path(first, explored, graph)
            p2, c2 = generate_path(first, explored2, graph)
            finalp = p1[:-1]+p2[::-1]
            calcedcost = c1+c2
            draw_final_path(ROOT, canvas, finalp, graph)
            print("The number of explored nodes of Bi-BFS: " + str(counter))
            print('The whole path:' + str(finalp))
            print('Length of path:', len(finalp))
            return finalp, calcedcost
         for each in graph[3][first]:
            if each not in explored.keys():
               for_f.append(each)
               explored[each] = first
               drawLine(canvas, *graph[5][first], *graph[5][each], col)
         counter+=1
         if counter % 1000 == 0:
            ROOT.update()
      ...
      while len(temp)>0:
         ...
         if first in for_f:
            ...
            return finalp, calcedcost
         for each in graph[3][first]:
            if each not in explored2.keys():
               back_f.append(each)
               explored2[each] = first
               drawLine(canvas, *graph[5][first], *graph[5][each], col)
         counter+=1
         if counter % 1000 == 0:
            ROOT.update()
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   hur = dist_heuristic(start, goal, graph)
   explored = {start:(0, [start])}
   frontier.push((hur, start, [start]))
   while not frontier.isEmpty():
      first = frontier.pop()
      if first[1]==goal:
         p = explored[first[1]][1]
         c = first[0]
         draw_final_path(ROOT, canvas, p, graph)
         print("The number of explored nodes of A star:" + str(track))
         print('The whole path:' + str(p))
         print('Length of the path:', len(p))
         return p, c
      for each in graph[3][first[1]]:
         costofp = explored[first[1]][0] + graph[4][(first[1], each)]
         if each not in explored or explored[each][0] > costofp:
            heuri = dist_heuristic(each, goal, graph)
            frontier.push((costofp+heuri, each, first[2]+[each]))
            explored[each] = (costofp, first[2]+[each])
            drawLine(canvas, *graph[5][first[1]], *graph[5][each], col)
      track+=1
      if track % 1000 == 0:
         ROOT.update()
   return None
...
def bi_a_star(start, goal, graph, col, ROOT, canvas, heuristic=dist_heuristic):
   ...
   counter = 0
   hur = dist_heuristic(start, goal, graph)
   frontier[0].push((hur, start))
   frontier[1].push((hur, goal))
   explored = [{start:(0, [start])}, {goal:(0, [goal])}]
   nodes = [{start}, {goal}]
   dest = [start, goal]
   switch = 1

   while not frontier[0].isEmpty() and not frontier[1].isEmpty():
      switch = 1-switch
      first = frontier[switch].pop()
      nodes[switch] -= {first[1]}
      if first[1] in nodes[1-switch]:
         path = explored[switch][first[1]][1][:-1] + explored[1-switch][first[1]][1][::-1]
         cost = explored[switch][first[1]][0] + explored[1-switch][first[1]][0]
         print("The number of explored nodes of Bi-A star:" + str(counter))
         print('The whole path:' + str(path))
         print('Length of the path:', len(path))
         return path, cost
      for each in graph[3][first[1]]:
         costofp = explored[switch][first[1]][0] + graph[4][(first[1], each)]
         if each not in explored[switch] or explored[switch][each][0]>costofp:
            hur2 = costofp + dist_heuristic(each, dest[1-switch], graph)
            frontier[switch].push((hur2, each))
            nodes[switch].add(each)
            explored[switch][each] = (costofp, explored[switch][first[1]][1]+[each])
            drawLine(canvas, *graph[5][first[1]], *graph[5][each], col)
      counter+=1
      if counter % 1000 == 0: ROOT.update()
   return None
...
def tri_directional(city1, city2, city3, graph, col, ROOT, canvas, heuristic=dist_heuristic):
   ...
   # Your code goes here
   firstpath, firstcost = bi_a_star(city1, city2, graph, col, ROOT, canvas)
   secondpath, secondcost = bi_a_star(city2, city3, graph, col, ROOT, canvas)
   thirdpath, thirdcost = bi_a_star(city3, city1, graph, col, ROOT, canvas)
   finalpath = min([(firstpath[:-1]+secondpath), (secondpath[:-1]+thirdpath), (thirdpath[:-1]+firstpath)])
   finalcost = min([(firstcost+secondcost), (secondcost+thirdcost), (thirdcost+firstcost)])
   draw_final_path(ROOT, canvas, finalpath, graph)
   return finalpath, finalcost
   
def main():
   start, goal = "Charlotte", "Los Angeles"
   third = "Chicago"
